...
namespace Ganpa.Serialization
{
    ...
    public static class JsonSerializerTester
    {
        ...
        public static void Assert(IJsonSerializer serializer)
        {
            ...
            if (sb.Length > 0)
                throw new GanpaException($"Serialization errors occurred:{Environment.NewLine}{sb}");
            ...
            using (var jsonStream = json.ConvertToStream())
            {
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    if (!json.Equals(actual))
                        throw new GanpaException(errorMessage);
                }

                if (toggleCollection.Features.Count != 2)
                    throw new GanpaException($"Different # of features: {errorMessage}");

                var feature1 = toggleCollection.Features.First();

                if (feature1.Name != "Feature1")
                    throw new GanpaException($"Wrong Feature1 name: {errorMessage}");

                if (feature1.Strategies.Count != 1)
                    throw new GanpaException($"Wrong stragegies count: {errorMessage}");

                if (feature1.Strategies[0].Name != "remoteAddress")
                    throw new GanpaException($"Wrong expected strategy name (remoteAddress): {errorMessage}");


                if (feature1.Strategies[0].Parameters.Count != 1)
                    throw new GanpaException($"Wrong expected strategy parameters count (1): {errorMessage}");

                var keyValuePair = feature1.Strategies[0].Parameters.First();
                if (keyValuePair.Key != "IPs")
                    throw new GanpaException($"Wrong expected strategy parameters key (IPs): {errorMessage}");

                if (keyValuePair.Value != "127.0.0.1")
                    throw new GanpaException($"Wrong expected strategy parameters value (127.0.0.1): {errorMessage}");
            }
        }
    }
}