    ...
    internal class CachedFilesLoader
    {
        ...
        public CachedFilesResult EnsureExistsAndLoad()
        {
            ...
            else
            {
                try
                {
                    result.InitialETag = fileSystem.ReadAllText(_etagFile);
                }
                catch (IOException ex)
                {
            ...
            if (!_fileSystem.FileExists(_toggleFile))
            {
                ...
                {
                    Logger.Error(() => $"UNLEASH: Unhandled exception when writing to toggle file '{_toggleFile}'.", ex);
                    _eventConfig?.RaiseError(new ErrorEvent() { Error = ex, ErrorType = ErrorType.FileCache });
                }
            }
            else
            {
                ...
                {
                    Logger.Error(() => $"UNLEASH: Unhandled exception when reading from toggle file '{_toggleFile}'.", ex);
                    _eventConfig?.RaiseError(new ErrorEvent() { Error = ex, ErrorType = ErrorType.FileCache });
                }
            }
